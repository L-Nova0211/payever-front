...
@Injectable()
export class PebClientCheckoutMicroLoaderService extends BaseMicroService {

  private buildHashObservables: { [key: string]: Observable<string> } = {};
  private buildMicroConfigObservables: { [key: string]: Observable<any> } = {};
  ...
  loadBuildMicroConfig(microCode: string): Observable<any> {
  ...
  loadMicroByScriptUrl(bootstrapScriptUrl: string): Observable<boolean> {
    return this.loadScript(bootstrapScriptUrl, null);
  }
  ...
  getResourceUrl(microCode: string, buildHash: string, resourceName: string, resourceType: string, allowCache: boolean = true): string {
    const config: any = this.config.frontend;
  ...
  private _createInnerMicroBuildObservable(microCode: string, innerMicroCode: string, subPath: string = null, forceReload: boolean = false): Observable<boolean> {
    return this.loadBuildHash(microCode, subPath).pipe(
      mergeMap((buildHash: string) => {
        return this._createBuildMicroConfigObservable(microCode, subPath).pipe(mergeMap((config: any) => {
  ...
  private _createBuildHashObservable(microCode: string, subPath: string = null): Observable<string> {
    if (this.registry.buildHashes[microCode]) {
      ...
      return this.httpClient.get(url).pipe(
        tap((hashmap: any) => this.registry.buildHashes[microCode] = hashmap),
        map(() => this.registry.buildHashes[microCode].micro),
        catchError(error => {
          console.error(`Cant load micro hash during ${(new Date()).getTime() - logStart}ms at '${url}':\n ${error.message || JSON.stringify(error)}`);
  ...
  private _createBuildMicroConfigObservable(microCode: string, subPath: string = null): Observable<any> {
    if (this.registry.buildMicroConfigs[microCode]) {
      ...
      return this.httpClient.get(url).pipe(
        ...
        catchError(error => {
          console.error(`Cant load micro config during ${(new Date()).getTime() - logStart}ms at '${url}':\n ${error.message || JSON.stringify(error)}`);
