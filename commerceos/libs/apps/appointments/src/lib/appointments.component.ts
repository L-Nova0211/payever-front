...
import { AppThemeEnum, PeBuilderEditorRoutingPathsEnum, PeDestroyService, PePreloaderService } from '@pe/common';
...
import { PeAppointmentsHeaderService, PeAppointmentsSidebarService } from './services';

const SIDENAV_NAME = 'app-appointments-sidenav';

@Component({
  selector: 'pe-appointments',
...
export class PeAppointmentsComponent implements OnDestroy, OnInit {
  public selectFolder: FolderItem<{ link: PeAppointmentsRoutingPathsEnum | PeBuilderEditorRoutingPathsEnum }>;

  public readonly mobileTitle$ = this.peAppointmentsHeaderService.mobileTitle$;
  public readonly sidebarTree = this.peAppointmentsSidebarService.createSidebar();
  ...
  private readonly toggleSidebar$ = this.peAppointmentsHeaderService.toggleSidebar$
    .pipe(
      tap(() => {
        this.toggleSidebar();
      }));
  ...
  constructor(
    ...
    private peAppointmentsSidebarService: PeAppointmentsSidebarService,
  ) {
    this.router.events
      .pipe(
  ...
  private addSidenavItem(): void {
    this.pePlatformHeaderService.assignSidenavItem({
      ...
      item: {
        title: this.translateService.translate('appointments-app.header.all_folders'),
        iconType: 'vector',
